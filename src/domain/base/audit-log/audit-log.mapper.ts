...
export class AuditLogMapper {
  ...
  static fromPgWithState(pg: AuditLogPg): AuditLog {
    return this.fromPg(pg).setPgState(this.toPg);
  }

  static fromPlain(plainData: AuditLogPlain): AuditLog {
    const plain: AuditLogPlain = {
      id: plainData.id,
  ...
  static fromJson(json: AuditLogJson): AuditLog {
    const plain: AuditLogPlain = {
      id: json.id,
  ...
  static toPg(domain: AuditLog): AuditLogPg {
    return {
      id: domain.id,
  ...
  static toJson(domain: AuditLog): AuditLogJson {
    return {
      id: domain.id,
  ...
  static toResponse(domain: AuditLog): AuditLogResponse {
    return {
      id: domain.id,
      createdAt: toISO(domain.createdAt),
      actorType: domain.actorType,
